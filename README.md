...
目前已有 A0／A1／A2／A3、B1／B2 入庫與庫存查詢、C1／C2 出庫與移位，以及本功能分支的 D1 盤點／損耗申請、D2 管理者審核與 D3 決策報表。D1／D2 尚未合併到 main，D3 接在同一功能分支。
...
登入後的共用導覽包含首頁、倉管操作，以及僅管理者可見的基本資料、審核申請與決策報表頁。倉管操作入口包含 B1 入庫、B2 庫存查詢、C1 出庫、C2 移位及 D1 盤點／損耗申請；管理者首頁從 D3 報表 API 取得即時統計。
...
- `products`：所有啟用品項各自的單位、目前總量、最低量、目標量、是否低庫存、參考補貨缺口、近 30 日出庫、歷來已核准的盤盈／盤虧／報廢量。沒有批次的啟用品項也列 0；低庫存是 `目前總量 < 最低量`，參考缺口是 `max(目標量 − 目前總量, 0)`。
...
1. 管理者登入首頁並點「更新統計」，核對有庫存品項、低庫存品項及待審申請數；再進「決策報表」，確認每個啟用品項分別顯示自身單位。可在「基本資料」新增一個最低量 2、目標量 5、單位「箱」的新啟用品項，報表更新後應列目前 0 箱、低庫存、參考缺口 5 箱，首頁低庫存品項數增加 1。
...
D3 測試使用暫存 SQLite，不改本機展示資料庫：`.\.venv\Scripts\python.exe -m pytest -q tests/test_d3.py`；完整回歸：`.\.venv\Scripts\python.exe -m pytest -q`；前端檢查：`npm.cmd --prefix frontend run build`。D1／D2 未合併時仍在 `feature/d-d1-adjustments` 續做 D3；跨 B／C 模組的完整展示在 A4 整合彩排再實測。
