...
登入後的共用導覽包含首頁、倉管操作，以及僅管理者可見的基本資料、審核申請與決策報表頁。倉管操作入口對倉管包含 B1 入庫、B2 庫存查詢、C1 出庫、C2 移位及 D1 盤點／損耗申請；管理者在此頁只能查詢庫存，不能提交倉管操作。管理者首頁從 D3 報表 API 取得即時統計。
...
先依「環境與安裝」及「SQLite 初始化」準備展示電腦。已有 `data/inventory.db` 時**不要再次初始化或把 seed 當作重置**；seed 只補缺，不還原已操作的餘量。所有下列動作都在同一台後端與同一個資料庫進行。先執行上方「同一網址展示」的建置與啟動指令，電腦開 `http://localhost:8000`，手機開 `http://<電腦區域網路 IPv4>:8000`。兩台裝置各自登入；手機要能連到這台電腦的 8000 埠。管理者審核頁為 `http://<主機>:8000/#reviews`，報表頁為 `http://<主機>:8000/#reports`。倉管操作頁對倉管提供五個快速跳轉；管理者在此頁只有庫存查詢，不顯示多餘的跳轉。管理者的基本資料、審核與報表頁依區塊提供快速跳轉；這些長頁面往下捲動後，右下角都會出現懸浮的「↑ 頂部」按鈕。快速跳轉、儲位明細、回頂部與倉管操作成功提示會以短暫緩動捲動，系統設定「減少動態效果」時不播放動畫。待審中的批次／儲位在操作選單中呈灰色且不可選，仍保留在查詢與歷史中。為避免 iPhone Safari 的原生下拉選單把頁面撐寬，出庫、移位及盤點申請選單只顯示精簡品項／儲位／批次 ID／數量；選取後會在表單內顯示完整品項與批次碼，操作紀錄也維持完整資訊。
...
| 庫存 CSV | 兩角色「倉管操作 → 庫存查詢 → 匯出庫存 CSV」；`GET /api/inventory/stock.csv` | 使用畫面目前的品項、批次、儲位條件，下載 UTF-8 CSV，含儲位數量、全批合計、入庫日、庫齡與效期；空結果只有標題列。文字欄位有試算表公式注入防護。 |
...
4. 在庫存查詢選品項或儲位，按「匯出庫存 CSV」。開啟下載檔，核對每列品項、批次、儲位與數量和查詢畫面一致；輸入不存在的批次碼再匯出，檔案應只有標題列。手機也要確認四個區塊無橫向捲動，並用電腦、手機連同一後端核對刷新結果。

自動檢查：`tests/test_extras.py` 使用暫存 SQLite 驗證權限、數量、報表分列、效期、CSV 與遷移保留資料；`tests/browser_extras.py` 以暫存資料庫檢查真實畫面與下載。執行 `npm.cmd --prefix frontend run build`、`.\.venv\Scripts\python.exe -m pytest -q`，以及安裝 Chrome 與 `tests/requirements-browser.txt` 後執行 `.\.venv\Scripts\python.exe -m tests.browser_extras`。實體 iPhone Safari 仍須本人驗收。
