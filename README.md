...
目前已有 A0／A1／A2／A3、B1 入庫與 C1／C2 出庫、移位。此 B2 功能分支加入庫存搜尋與異動歷史；盤點、損耗與報表仍待其他片段整合。
...
登入後的共用導覽包含首頁、倉管操作，以及僅管理者可見的基本資料頁。倉管操作入口包含 B1 入庫、B2 庫存查詢、C1 出庫與 C2 移位，並預留 D 的盤點頁；管理者首頁保留 D3 統計位置，未合併前明確顯示待串接。
...
目前可用兩台裝置登入後讀取同一資料庫；倉管可操作入庫、出庫與移位，兩角色可用 B2 查詢庫存與異動。盤點與審核待 D 片段整合。
