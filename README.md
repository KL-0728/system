...
管理者登入後點上方「審核申請」，或直接開啟 `http://localhost:5173/#reviews`。頁面分列所有待審申請與最近 100 筆已審核申請；已核准／駁回的項目會從待審清單移到已審核清單，重新整理也能找回，不會刪除。點「查看詳情與審核」可核對送件人、品項、批次、儲位、原數、目前餘量、盤點實數或報廢量、預計差額、原因與時間。倉管開啟該網址會看到「權限不足」，且後端管理 API 回 403；倉管可在「我的最近申請」看到自己申請的審核結果及駁回原因。若前端已更新，但審核頁顯示 `request_id` 不能解析為整數，代表仍在執行未註冊 `/api/adjustments/reviewed` 的舊後端；先停止並重新啟動後端，再刷新網頁，不需重建或清除資料庫。
