...
export default function SettingsPage() {
  ...
  function editProduct(item: Product) { setProductMessage(''); setProductId(item.id); setProduct({ name: item.name, unit: item.unit, min_qty: item.min_qty, target_qty: item.target_qty, is_active: item.is_active }); }
  function editLocation(item: Location) { setLocationMessage(''); setLocationId(item.id); setLocation({ warehouse_id: item.warehouse_id, number: String(Number(item.code.split('-').at(-1))), is_active: item.is_active }); }
  const selectedWarehouse = warehouses.find((item) => item.id === location.warehouse_id);
  const locationPreview = selectedWarehouse && location.number
