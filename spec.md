...
若某項無法用展示當天的設備、網址與示範資料**實際操作出結果**，就留在構想中，不列為已完成功能。加做功能需先完成自己的簡短驗收，再納入展示。缺貨詢問量不得混入出庫量或扣減庫存；簡圖只是代碼方格，並非真實建築平面圖；CSV 要與畫面最後一次成功查詢所套用的篩選條件一致，未按「查詢／更新」的輸入變更不影響匯出。
...
- 本次所有數量都是**非負整數**。各品項自訂一個顯示單位（如籠、箱、整公斤），不換算、不接收小數。若要稱到 0.5 公斤，須將整個品項改以「公克」記錄，或列入未來擴充；已有庫存異動或缺貨需求紀錄的品項不得直接改單位，避免既有紀錄被解讀成新單位。
...
- 刪除有紀錄的品項、儲位或使用者不列入功能；停用後保留歷史。停用儲位前要先搬空，且該儲位不得有待審申請；既有異常資料中，停用儲位的盤盈核准仍須被拒絕，駁回則可結案。資料庫檔 `data/inventory.db` 不提交 Git，使用 `backend/seed.py` 重新產生示範資料。
